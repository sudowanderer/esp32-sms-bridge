...
当前硬件使用 ESP32C3 通过 UART 与 ML307R-DC 4G 模块通信，并用一个 GPIO 控制 4G 模块 EN 引脚。
...
- 当前硬件方案中，ML307R 的 EN 不再固定短接，而是由 ESP32C3 的 GPIO5 控制，方便固件重启模组。
...
当前代码通过 `MODEM_EN_PIN` 控制 ML307R-DC 的 EN 引脚。
...
4. 调用 `modemPowerCycle()`，通过 EN 引脚重启 ML307R。
...
ML307R 收到短信后，通过 UART 主动上报。当前代码使用 `AT+CNMI=2,2,0,0,0`，新短信会直接发送到串口。
...
- [ ] GPIO5 能正确控制 ML307R EN，下电再上电后模组可恢复 AT 响应。
- [ ] `AT+CMGF=0` 设置成功。
