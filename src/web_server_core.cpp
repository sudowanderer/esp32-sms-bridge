  ...
  switch (page) {
    case WebPageKind::Status:
      writer.append(
          ...
          "function sig(q){return ['excellent','good','fair','weak'].includes(q)?q:'unknown';}"
          ...
          "const q=sig(j.cellular.rsrp_quality);$('signal_icon').className='signal-icon signal-'+q;$('modem_rsrp_text').textContent=j.cellular.lte_signal_known?j.cellular.rsrp_dbm+' dBm ('+j.cellular.rsrp_quality+')':'unknown';$('modem_rsrq').textContent=j.cellular.lte_signal_known?rsrq(j.cellular.rsrq_db_tenths):'unknown';$('modem_cesq').textContent=val(j.cellular.cesq_raw);"
...
bool webBuildPageHtml(WebPageKind page, char* output, size_t outputSize) {
  ...
  writer.append(
      ...
      ".signal-wrap{display:inline-flex;align-items:center;gap:8px}"
      ".signal-icon{display:inline-flex;align-items:flex-end;gap:2px;width:24px;height:15px;vertical-align:-2px}"
      ".signal-bar{display:block;width:4px;background:#cbd5e1;border-radius:1px}"
      ".signal-bar:nth-child(1){height:4px}.signal-bar:nth-child(2){height:7px}.signal-bar:nth-child(3){height:10px}.signal-bar:nth-child(4){height:13px}"
      ".signal-weak .signal-bar:nth-child(1),.signal-fair .signal-bar:nth-child(-n+2),.signal-good .signal-bar:nth-child(-n+3),.signal-excellent .signal-bar{background:#2f80ed}"
      ".signal-weak .signal-bar:nth-child(1){background:#e0a106}"
      ".signal-unknown .signal-bar{background:#cbd5e1}"
  ...
  switch (page) {
    case WebPageKind::Status:
      writer.append(
          ...
          "<tr><td>Signal RSRP</td><td><span class=\"signal-wrap\"><span id=\"signal_icon\" class=\"signal-icon signal-unknown\" aria-hidden=\"true\"><span class=\"signal-bar\"></span><span class=\"signal-bar\"></span><span class=\"signal-bar\"></span><span class=\"signal-bar\"></span></span><span id=\"modem_rsrp_text\">loading</span></span></td></tr>"
